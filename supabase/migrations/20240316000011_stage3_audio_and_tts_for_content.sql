-- Add stage3_audio for storing TTS URLs per sentence (correct/incorrect arrays)
alter table public.vocabulary
  add column if not exists stage3_audio jsonb not null default '{"correct":[],"incorrect":[]}'::jsonb;

-- Update get_active_pool to return stage3_audio (must drop first to change return type)
...
begin
  ...
  select v.id, v.word, v.status, v.cycle, v.stage, v.consecutive_correct,
         ...
         coalesce(v.stage3_audio, '{"correct":[],"incorrect":[]}'::jsonb),
