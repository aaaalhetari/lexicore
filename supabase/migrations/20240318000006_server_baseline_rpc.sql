-- Server baseline RPC (functions-only): final canonical definitions.
-- Note: uses create-or-replace only, no schema/table rename operations.
...
begin
  ...
  select coalesce(
    ...
    '{"cycle_1":{"stage_1_required":4,"stage_2_required":4,"stage_3_required":4},
      ...
      "new_words_per_day":25,"waiting_target":50}'::jsonb
  ) into v_settings;
...
-- 5. Update check_refill_needed to use card_jobs
...
-- Preferred naming (compat): use check_card_jobs_needed going forward.
create or replace function public.check_card_jobs_needed(p_user_id uuid)
returns void as $$
begin
  perform public.check_refill_needed(p_user_id);
end;
$$ language plpgsql security definer set search_path = '';
...
grant execute on function public.claim_card_jobs(int, uuid) to service_role;
grant execute on function public.reset_stuck_card_jobs() to service_role;
grant execute on function public.complete_card_job(bigint) to service_role;
grant execute on function public.fail_card_job(bigint) to service_role;