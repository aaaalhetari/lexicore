-- Add dedicated audio columns (audio_word, audio_stage1_definitions, etc.)
-- Migrate from embedded audio_url in JSONB and stage3_audio

alter table public.vocabulary
  add column if not exists audio_word text,
  add column if not exists audio_stage1_definitions jsonb not null default '[]'::jsonb,
  add column if not exists audio_stage2_sentences jsonb not null default '[]'::jsonb,
  add column if not exists audio_stage3_correct jsonb not null default '[]'::jsonb,
  add column if not exists audio_stage3_incorrect jsonb not null default '[]'::jsonb;

-- Migrate audio_url -> audio_word
update public.vocabulary set audio_word = audio_url where audio_url is not null and audio_word is null;

-- Migrate stage1_definitions[].audio_url -> audio_stage1_definitions array (preserve indices)
update public.vocabulary v set audio_stage1_definitions = (
  select coalesce(jsonb_agg(coalesce(elem->>'audio_url', '') order by ord), '[]'::jsonb)
  from jsonb_array_elements(v.stage1_definitions) with ordinality arr(elem, ord)
)
where jsonb_array_length(stage1_definitions) > 0;

-- Migrate stage2_sentences[].audio_url -> audio_stage2_sentences array (preserve indices)
update public.vocabulary v set audio_stage2_sentences = (
  select coalesce(jsonb_agg(coalesce(elem->>'audio_url', '') order by ord), '[]'::jsonb)
  from jsonb_array_elements(v.stage2_sentences) with ordinality arr(elem, ord)
)
where jsonb_array_length(stage2_sentences) > 0;

-- Migrate stage3_audio -> audio_stage3_correct, audio_stage3_incorrect
update public.vocabulary v set
  audio_stage3_correct = coalesce(v.stage3_audio->'correct', '[]'::jsonb),
  audio_stage3_incorrect = coalesce(v.stage3_audio->'incorrect', '[]'::jsonb)
where v.stage3_audio is not null;

-- Update get_active_pool to return new audio columns
...
-- Update check_refill_needed to use audio_word for audio jobs
...
begin
  ...
  -- Audio: create jobs for words without audio_word (or audio_url for backward compat)
  insert into public.refill_jobs (user_id, job_type, payload)
  select v.user_id, 'audio', jsonb_build_object('word_id', v.id, 'word', v.word)
  from public.vocabulary v
  where v.user_id = p_user_id
    and coalesce(v.audio_word, v.audio_url) is null
