-- B1-friendly: refill_jobs → card_jobs

-- 1. Rename table
alter table public.refill_jobs rename to card_jobs;

-- 2. Rename constraint
alter table public.card_jobs rename constraint refill_jobs_job_type_check to card_jobs_job_type_check;

-- 3. Rename index
alter index if exists refill_jobs_pending_idx rename to card_jobs_pending_idx;

-- 4. Update RLS policies
drop policy if exists "refill_jobs_select_own" on public.card_jobs;
create policy "card_jobs_select_own" on public.card_jobs for select using (auth.uid() = user_id);
