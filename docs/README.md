...
**جذر المستودع على GitHub:** استنسخ المشروع وافتح في Cursor/VS Code **المجلد الذي يحتوي `package.json`** (نفس مستوى `src/` و`supabase/`). Git والنشر هما لهذا الجذر فقط. إعدادات المحرر (مثل العربية في المحرر) في `.vscode/` هنا.
