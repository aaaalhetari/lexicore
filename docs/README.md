...
**جذر المستودع على GitHub:** استنسخ المشروع وافتح في Cursor/VS Code **المجلد الذي يحتوي `package.json`** (نفس مستوى `src/` و`supabase/`). مجلد أعلى مثل `lexicore_vue` على جهازك قد يكون فقط غلافًا محليًا؛ Git الرسمي والنشر هما لمحتويات هذا الجذر فقط. إعدادات المحرر (مثل العربية في المحرر) موجودة في `.vscode/` هنا.
